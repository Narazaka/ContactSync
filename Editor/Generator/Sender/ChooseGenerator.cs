...
namespace Narazaka.VRChat.ContactSync.Editor.Generator.Sender
{
    class ChooseGenerator : SubSenderGenerator
    {
        ...
        protected override void GenerateAnimator()
        {
            ...
            {
                var value = Component.AllowUnconstrained ? i + 1 : i;
                var state = layer.AddNewState($"Choice {i}").Position(0, i * 100).CreateClip(Name($"Choice {i}"), clip => clip.Active(ContactPath, true).Position(ContactPath, Constants.Choose.Min.CenterPosition + Constants.Choose.Step * i));
